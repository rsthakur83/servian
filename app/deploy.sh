...
if [ "$lc" == "$lcfg1" ];then
	...
	sg=`aws ec2  describe-security-groups --filter Name=vpc-id,Values=$vpc  Name=tag:Name,Values='APP SG' --region us-east-1 --query 'SecurityGroups[*].[GroupId]' --output text`
	#sg=`aws ec2  describe-security-groups --filter Name=vpc-id,Values=$vpc  Name=group-name,Values='APP Security Group' --region aws-region --query 'SecurityGroups[*].[GroupId]' --output text`
